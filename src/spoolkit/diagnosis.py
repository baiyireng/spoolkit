...
def key_path() -> Path:
    ...
    改名兼容：旧位置（`~/.agents-dev/diagnosis.key`）**还认**。换位置就不认旧的，
    等于把已经签出去的报告全变成"来源无法验证"——那是我们改名造成的损失，
    不该由用户承担。
    """
    override = os.environ.get("SPOOLKIT_DIAGNOSIS_KEY_PATH") or os.environ.get(
        "AGENTS_DEV_DIAGNOSIS_KEY_PATH"
    )
    if override:
        return Path(override)
    current = Path.home() / ".spoolkit" / "diagnosis.key"
    if current.exists():
        return current
    legacy = Path.home() / ".agents-dev" / "diagnosis.key"
