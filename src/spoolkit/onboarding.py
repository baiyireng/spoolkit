...
`spool run --goal "..."` 会回一句"假模型需要 --script"，而真正该说的是
...
- **密钥不写进配置文件**。配置会被 `spool config` 打印出来，密钥写进去
  就等于每次显示一遍。向导把密钥写进项目根的 `.env`（已在 .gitignore 里），
  那里本来就是文档里说的位置。

每一条都可以**留空跳过**：现在不想填就跳过，回头用 `spool config` 或
...
def run_wizard(
    ...
    if provider == "llamacpp":
        ...
        say('  spool run --provider fake --script script.json --goal "..."')

    written = settings.save({**settings.load(), **values})
    say(f"已写入 {written}")
    say("以后用 `spool config` 看生效值和来源；这次不再问。")
    return values
