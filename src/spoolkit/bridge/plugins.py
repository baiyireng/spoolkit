...
1. **装好的包**：entry point 组 `spoolkit.channels`（`pyproject.toml` 里声明
   `[project.entry-points."spoolkit.channels"]`）。适合正经发布出去的适配器。
2. **没打包的脚本**：环境变量 `SPOOLKIT_CHANNEL_PLUGINS`（改名前的
   `AGENTS_DEV_CHANNEL_PLUGINS` 也认）给一串
...
ENTRY_POINT_GROUP = "spoolkit.channels"
# 新名在前，旧名照认（改名兼容）。
ENV_VAR = "SPOOLKIT_CHANNEL_PLUGINS"
LEGACY_ENV_VAR = "AGENTS_DEV_CHANNEL_PLUGINS"
...
def load_channel_factories() -> dict[str, Callable[[dict], object]]:
    ...
    spec = os.environ.get(ENV_VAR) or os.environ.get(LEGACY_ENV_VAR) or ""
    factories.update(_from_env(spec))
    return factories
